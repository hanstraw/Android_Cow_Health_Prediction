...
import org.pytorch.LiteModuleLoader // 替换为 Lite 版的 ModuleLoader
import org.pytorch.Tensor
import org.pytorch.Module

fun loadModel(context: Context): Module {
    // 使用 PyTorch Lite 加载模型
    return LiteModuleLoader.load(assetFilePath(context, "cow_health_model.ptl"))  // .ptl 为 Lite 版模型格式
}

fun runModelPrediction(context: Context, inputs: FloatArray): Float {
    // 1. 加载模型
    ...
    val scales = floatArrayOf(
        ...
        2.0953205F, 126.76042F, 38.759575F
    )   // 替换为提取的标准差
    ...
    // 返回预测结果
    return if (outputData[0] > 0.5) 0f else 1f
}
